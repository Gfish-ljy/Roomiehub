...
**RoomieHub** 是一款面向宿舍场景的 Android 环境监测与健康评分应用。通过手机传感器或局域网内的 Spring Boot 服务，实时采集温度、湿度、噪音、光照、空气质量等数据，并给出健康评分、趋势分析与智能建议。
...
| 项目 | 说明 |
|------|------|
| 登录 | 当前为本地 Mock，未对接真实 Spring Boot 用户认证 |
| 手机模式 | 温湿度为模拟值，AQI 为估算值 |
| 服务器模式 | 需自行部署 Spring Boot 与 ESP32 采集端 |
| 多设备 | 暂无云端同步，数据仅存本机 Room |
| 管理端 | 暂无独立管理员 APP，管理功能在电脑端完成 |

可扩展方向：真实登录 API、WebSocket 实时推送、多宿舍管理、导出报告 PDF 等。

---

## 许可证

本项目为课程 / 实训用途。如需商用或二次发布，请自行补充许可证并与相关硬件、服务端方案一并评估。
